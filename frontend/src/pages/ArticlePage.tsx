...
// Date formatting: YYYY-MM-DD → locale-aware long date
      ...
      <dl className="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
        ...
        {article.author && (
          <div className="py-2.5 flex gap-4">
            <dt className="w-40 shrink-0 text-slate-500 dark:text-slate-400">{t('article.metaAuthor')}</dt>
        ...
        {article.journal && (
          <div className="py-2.5 flex gap-4">
            <dt className="w-40 shrink-0 text-slate-500 dark:text-slate-400">{t('article.metaJournal')}</dt>
            <dd className="text-slate-900 dark:text-slate-100 italic">{article.journal}</dd>
          </div>
        )}

        <div className="py-2.5 flex gap-4">
          <dt className="w-40 shrink-0 text-slate-500 dark:text-slate-400">{t('article.metaDate')}</dt>
          <dd className="text-slate-900 dark:text-slate-100">{formatDate(article.publication_date, i18n.language)}</dd>
        </div>

        {article.affiliation_country && (
          <div className="py-2.5 flex gap-4">
            <dt className="w-40 shrink-0 text-slate-500 dark:text-slate-400">{t('article.metaCountry')}</dt>
        ...
        {article.cited_by_count != null && (
          <div className="py-2.5 flex gap-4">
            <dt className="w-40 shrink-0 text-slate-500 dark:text-slate-400">{t('article.metaCitations')}</dt>
        ...
        {article.doi && (
          <div className="py-2.5 flex gap-4">
            <dt className="w-40 shrink-0 text-slate-500 dark:text-slate-400">{t('article.metaDoi')}</dt>
            <dd>
              <a
