...
// Нижний бар останавливается в 88% пути к target — отличается от него
// не больше чем на ~12% (в рамках требуемых 10-15%), не сливаясь с ним полностью.
const RANKED_FADE_MAX_T = 0.88;

/**
 * Цвет бара горизонтального ранжированного чарта: верхний ранг (index=0)
 * остаётся насыщенным `base`. Нижние ранги — самые короткие бары, поэтому
 * вместо затухания в сторону низкой контрастности (что сделало бы их ещё
 * менее заметными при уже маленькой площади) цвет наоборот смещается к
 * точке максимального контраста с фоном панели (Sheet/popover) текущей
 * темы: к белому в dark (фон панели — тёмно-синий #152236) и к чёрному в
 * light (фон панели — чистый белый). Так короткие нижние бары остаются
 * так же хорошо читаемыми, как и длинный верхний.
 */
export function getRankedBarColor(
  ...
  const t = total <= 1 ? 0 : (index / (total - 1)) * RANKED_FADE_MAX_T;
  return mixHex(colors.base, target, t);
}
