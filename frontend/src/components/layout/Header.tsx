    ...
    >
      <div className="mx-auto flex h-14 max-w-screen-xl items-center justify-between px-4">
        ...
        {/* Правый кластер шапки. ThemeToggle/LanguageSwitcher — ВСЕГДА видимы
            ...
            уже); дублируется в MobileNavSheet (бургер, ниже) для <sm. */}
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          {/* Бургер — виден только <sm (кнопка сама скрывается через sm:hidden
              в MobileNavSheet); теперь содержит только nav-ссылки + auth,
              ThemeToggle/LanguageSwitcher уже вынесены выше в постоянно
              видимую часть кластера. */}
          <MobileNavSheet />
        </div>
      </div>
    </header>
