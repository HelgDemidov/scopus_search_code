  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        data-side={side}
        className={cn(
          // ВАЖНО: размер (h-*/w-*/max-w-*) сюда намеренно НЕ включён — раньше
          // data-[side=X]:h-full/w-3/4/sm:max-w-sm перебивали по CSS-специфичности
          // любой размерный класс, переданный потребителем через `className` (см.
          // docs/explore-analytics/explore-charts-refactor/spec.md §2). Каждый потребитель SheetContent
          // обязан сам задать w-*/h-*/max-w-* в своём className.
          "fixed z-50 flex flex-col gap-4 bg-popover bg-clip-padding text-sm text-popover-foreground shadow-lg transition duration-200 ease-in-out data-[side=bottom]:inset-x-0 data-[side=bottom]:bottom-0 data-[side=bottom]:border-t data-[side=left]:inset-y-0 data-[side=left]:left-0 data-[side=left]:border-r data-[side=right]:inset-y-0 data-[side=right]:right-0 data-[side=right]:border-l data-[side=top]:inset-x-0 data-[side=top]:top-0 data-[side=top]:border-b data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[side=bottom]:data-[state=open]:slide-in-from-bottom-10 data-[side=left]:data-[state=open]:slide-in-from-left-10 data-[side=right]:data-[state=open]:slide-in-from-right-10 data-[side=top]:data-[state=open]:slide-in-from-top-10 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[side=bottom]:data-[state=closed]:slide-out-to-bottom-10 data-[side=left]:data-[state=closed]:slide-out-to-left-10 data-[side=right]:data-[state=closed]:slide-out-to-right-10 data-[side=top]:data-[state=closed]:slide-out-to-top-10",
          className
        )}
        {...props}
      >
