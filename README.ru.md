...
*   **Результат:** пороги не пройдены — `P(95) = 11.89с`, `P(99) = 13.39с`, но **0% ошибок**
    (ни одного тайм-аута или упавшего запроса — чистая очередь, а не артефакты
    пула-соединений/сети из прошлых попыток). Это настоящий, воспроизводимый предел
    масштабирования.
*   **Root cause (подтверждено `EXPLAIN ANALYZE`):** оба эндпоинта скатываются в полный
    параллельный seq scan — ни одному не подходит ни один существующий индекс под фактическую
    форму запроса:
    - `/articles/?search=` — `title ILIKE '%term%' OR author ILIKE '%term%'`. Ведущий wildcard
      отключает любой btree-индекс на таблице (включая существующие `ix_articles_lower_*`) —
      нужен `pg_trgm` GIN-индекс. COUNT(*) для пагинации — доминирующая стоимость сам по себе:
      **284мс/запрос**, parallel seq scan, без `LIMIT`, который мог бы прервать сканирование раньше.
    - `/stats/journal-impact` — `EXTRACT(year FROM publication_date) <= max_year` — функция над
      колонкой, тоже отключает индексацию: **112мс/запрос** (parallel seq scan + join с
      `catalog_articles`).
    - По отдельности оба — доли секунды, незаметно в браузере. При 20 конкурентных VU каждый
      запрос поднимает свои параллельные воркеры, конкурирующие за CPU-ядра контейнера — именно
      эта очередь, а не стоимость одного запроса, даёт многосекундный хвост latency.
*   **Известный пробел / кандидат в следующий тикет:** добавить `pg_trgm` GIN-индекс на
    `title`/`author` (или перейти на полнотекстовый поиск Postgres / `tsvector`) и функциональный
    индекс под year-фильтр, затем перемерить. Осознанно не реализовано в рамках этого тикета —
    его объём был «измерить и задокументировать», а не «заставить планировщик запросов делать
    что-то принципиально иное»; изменение того, как работает поиск — решение для отдельного
    тикета, а не правка походя.
