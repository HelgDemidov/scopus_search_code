...
Актуальная версия миграций: `0019_seeder_run_state`.
...
7. Удаляются осиротевшие `articles` (`POST /seeder/gc`), затем проверяется счётчик прогонов (`POST /seeder/vacuum`) — раз в 10 прогонов выполняется `VACUUM ANALYZE articles`.
...
**Бэкенд:** 332 теста (`pytest` + `pytest-asyncio`), все зелёные, три слоя:

| Слой | Тестов | Что проверяет |
|---|---|---|
| Unit (SQLite, мокированный) | 145 | Сервисы (article, catalog, search, user), Scopus-клиент, контракты интерфейсов, seeder router/keyword generator, Redis-кэш, Sentry-конфигурация |
| Integration (SQLite) | 159 | Полный HTTP-стек: auth, статьи, история поисков, сброс пароля, RT-жизненный цикл, seeder endpoint, observability/Sentry capture |
| Integration (PG) | 28 | Конкурентность `pg_advisory_xact_lock`, фильтрация каталога по `search=`, `VACUUM ANALYZE` через `POST /seeder/vacuum`; требует `DATABASE_TEST_URL` (throwaway PG, никогда не Supabase) |
...
**Дополнение — переоценка write-cost перед разморозкой сидера:** реальная статистика прода (`pg_stat_user_tables`) показала, что 92.3% исторических апдейтов сидера на `articles` были HOT-апдейтами — повторное обнаружение уже известной статьи переписывает байт-в-байт те же значения title/author, и Postgres пропускает обслуживание индексов вообще, независимо от их типа. Реальный риск оказался у́же и был измерен на изолированной, одноразовой ветке Neon (те же данные, переведены на GIN, удалена после эксперимента): pending-buffer GIN (`fastupdate`, `gin_pending_list_limit` — 4МБ по умолчанию, подтверждено идентичным и на Supabase) делает чтение каталога прогрессивно медленнее по мере накопления новых строк — примерно ×2 уже за ~2 суток при историческом темпе прироста сидера, а планировщик может вовсе бросить trgm-индекс в пользу полного последовательного скана (~×20) до следующего `VACUUM`. Фикс: `POST /seeder/vacuum` теперь выполняет `VACUUM ANALYZE articles` раз в 10 прогонов сидера (~20ч при возобновлённом расписании раз в 2 часа) — с запасом внутри этого окна; штатный `autovacuum_vacuum_insert_threshold` сработал бы сам не раньше ~13 дней при этом темпе — слишком медленно, чтобы полагаться только на него. Сидер снова работает по расписанию раз в 2 часа.
