...
|---|---|
| **Без авторизации** | Просмотр и поиск тематической коллекции «AI & Neural Network Technologies» (~39 500 публикаций); многокритериальная фильтрация по году, стране, типу документа и статусу open access; детальные страницы статей; аналитика коллекции (тренды по годам, географии, типам документов, ведущим журналам и ключевым словам) |
...
│                     #   statsStore, tokenStore (AT in-memory, без localStorage)
...
Актуальная версия миграций: `0013_fix_schema_drift`.
...
**Бэкенд:** 154 теста (`pytest` + `pytest-asyncio`), все зелёные, три слоя:

| Слой | Тестов | Что проверяет |
|---|---|---|
| Unit (SQLite, мокированный) | ~40 | Сервисы (article, catalog, search, user), Scopus-клиент, контракты интерфейсов, seeder router |
| Integration (SQLite) | ~96 | Полный HTTP-стек: auth, статьи, история поисков, сброс пароля, RT-жизненный цикл, seeder endpoint |
| Integration (PG) | 18 | Конкурентность `pg_advisory_xact_lock`; требует `DATABASE_TEST_URL` (throwaway PG, никогда не Supabase) |
| E2E (Staging) | — | Реальный Railway + Supabase staging; пропускается без `E2E_BASE_URL` |

**Фронтенд:** 181 тест (`Vitest` + Testing Library), все зелёные; покрытие statements 76.5% (порог: 70%).
