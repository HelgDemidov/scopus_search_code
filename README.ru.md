...
- **Два режима поиска** — бесплатный каталог из ~350 тыс. статей или live-запросы по всему Scopus в рамках личной недельной квоты
...
- **Продакшн-инженерия** — cache-aside на Redis, триграммный полнотекстовый поиск, структурные логи + Sentry-трейсинг, реальные задокументированные инциденты ниже
...
<table>
<tr>
<th>Без авторизации</th>
<th>С авторизацией</th>
</tr>
<tr>
<td>

- Просмотр и поиск тематической коллекции «AI & Neural Network Technologies» (~350 000 публикаций)
- Многокритериальная фильтрация по году, стране, типу документа и статусу open access
- Детальные страницы статей
- Интерактивный аналитический дашборд (`/explore`): cross-filter графики, конструктор pivot-таблиц Table Builder (метрика — количество или среднее число цитирований), Journal Landscape scatter
- Статистика по трендам публикаций, географии, типам документов, ведущим журналам, авторам и ключевым словам

</td>
<td>

- Все возможности из левой колонки, плюс:
- Live-поиск по всей базе Scopus (до 25 результатов за запрос)
- Личная история поисков с фильтрацией и персональная аналитика (`/explore?mode=personal`)
- Счётчик недельной квоты API
- Управление аккаунтом (email/пароль · Google OAuth · сброс пароля по email)
...
| `GET` | `/articles/stats/summary` | Лёгкие сводные KPI-показатели (6 скаляров) для плиток заголовка `/explore` |
...
| `POST` | `/seeder/refresh-stats-cache` | Принудительно обновить Redis-кэш `/explore` (stats/KPI-показатели/journal-impact) сразу после изменения каталога сидером |
...
|---|---|---|
| `articles` | Нормализованный реестр публикаций Scopus | ~350 700 |
| `catalog_articles` | Принадлежность статей к тематической коллекции (keyword сидера) | ~350 000 |
...
5. Бэкенд запрашивает Scopus и атомарно выполняет upsert статей в `articles` + `catalog_articles`, возвращает `rate_remaining`.
...
*   **Первое честное измерение:** пороги не пройдены — `P(95) = 11.89с`, `P(99) = 13.39с`, но
    **0% ошибок** (чистая очередь, а не артефакты пула-соединений/сети из более ранней, баганной
    ...
    seq scan — ни одному не подходил ни один существующий индекс под фактическую
...
*   **Исправлено за 3 измеренных шага, от дешёвого к дорогому** (полное обсуждение компромиссов —
...
Исправили добавлением `db_namespace` (sha256 от `DATABASE_URL`) в каждый ключ кэша.
...
Баг нашли, написав одноразовый скрипт, который вызывал репозиторий напрямую против продакшн-БД, и воспроизвели точно тот же traceback, что и в браузере. Исправили явным `.select_from(SearchResultArticle)`.
