...
**Бэкенд:** 286 тестов (`pytest` + `pytest-asyncio`), все зелёные, три слоя:

| Слой | Тестов | Что проверяет |
|---|---|---|
| Unit (SQLite, мокированный) | 113 | Сервисы (article, catalog, search, user), Scopus-клиент, контракты интерфейсов, seeder router, Redis-кэш |
| Integration (SQLite) | 150 | Полный HTTP-стек: auth, статьи, история поисков, сброс пароля, RT-жизненный цикл, seeder endpoint |
...
**Фронтенд:** 782 теста (`Vitest` + Testing Library), все зелёные; покрытие statements 85.9% (порог: 70%).
