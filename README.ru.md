...
- сервис приложения `app` остается активным;
- сервис локальной базы `db` может быть отключен (закомментирован в конфигурации).
...
- `GET /articles/?page=1&size=10` — список сохраненных статей из локальной БД с пагинацией (публичный эндпоинт)
