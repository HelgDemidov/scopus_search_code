...
def _build_prompt(query: str, grounding: PivotGroundingContext) -> str:
    ...
    return (
        ...
        "- NEVER set row_dim or col_dim to the same dimension as filter_dim. If a named value "
        "narrows the data (-> filter_dim/filter_value) and the question otherwise implies only "
        "ONE axis, pick the SECOND axis from a DIFFERENT dimension — this applies even when the "
        "value is stated without an explicit preposition (a trailing entity name, not just "
        "'from X'/'in X'). Examples: 'articles from China by year' -> row_dim='year', "
        "col_dim='doc_type' (NOT 'country'), filter_dim='country', filter_value='China'. "
        "'average citations by document type, China' -> row_dim='doc_type', col_dim='year' "
        "(NOT 'country'), filter_dim='country', filter_value='China', metric='avg_citations'.\n"
