...
Chain: `f9a3c1e2b7d4` → `0010` → `0011` → `0012` → `0013_fix_schema_drift` → `0014_functional_indices_lower` → `0015_trim_search_history_over_limit` → `0016_trgm_gist_search_indices` → `0017_publication_date_index` → `0018_trgm_gin_indices` → `0019_seeder_run_state` (head; single-row counter table driving `POST /seeder/vacuum`, see Catalog search latency below). Staging migrated manually before merge, prod gets each migration automatically on next deploy via `entrypoint.sh` — same pattern since 0016/0017/0018.
...
Root cause of `GET /articles/?search=` taking 0.5–9.8s: `get_total_count()`'s `BitmapOr` scan over `pg_trgm` GiST indices was CPU-bound (~4.9s, 100% buffer cache hits — confirmed `VACUUM ANALYZE` alone doesn't help, it's not a cache-miss/stale-stats problem). Fix: (1) Redis cache-aside on `get_total_count` (TTL=60s, `make_catalog_count_cache_key`, same pattern as `get_stats`) — `get_all()` stays uncached/live; (2) `ix_articles_title_trgm`/`ix_articles_author_trgm` switched **GiST → GIN** (migration `0018_trgm_gin_indices`) — reverses the PR #58 GiST choice. Measured on prod after deploy, warm: `get_total_count()` 4.9s → 164ms (~30x); `get_all()` (untouched by this PR) also dropped 520ms → 75ms as an unplanned side effect of freed `shared_buffers` room — combined ~240ms vs the original 5.4-10.7s. Storage: `articles`+`catalog_articles` 281MB → 189.5MB (-32.6%), ~99% of the saving from the two trgm indices alone (129→46.7MB, 16→7.14MB). `ix_articles_author_trgm` investigated for removal (looked low-value on one test term) — kept: a 20-term sample showed massive author-only recall on common surnames (e.g. "wang" — 8353 matches only via author, none via title).
**Write-cost re-check before unfreezing the seeder (08-03):** GIN's write cost was PR #58's original reason to prefer GiST — re-checked, not assumed, before resuming the seeder's 2h cron. Prod `pg_stat_user_tables` showed 92.3% of historical `articles` updates were HOT (byte-identical re-writes touch no index regardless of type). Real risk, measured on a disposable Neon branch (deleted after): GIN's `fastupdate` pending-list buffer (`gin_pending_list_limit`=4MB, confirmed on Supabase) degrades reads ~2x within ~2 days of un-vacuumed inserts at the seeder's historical growth rate, occasionally worse (planner drops the index for a full Seq Scan). Fix: `POST /seeder/vacuum` (`seeder_run_state` table, migration `0019`) runs `VACUUM ANALYZE articles` every 10 seeder runs (~20h at the resumed 2h cadence) — well inside that window; `autovacuum_vacuum_insert_threshold` alone would only fire after ~13 days. `seeder.yml` is back on its `0 */2 * * *` schedule. Full experiment writeup in `docs/backend-performance/catalog-search-latency/spec.md` §5 (local only, not in git).
...
## OpenAlex PDF Feed (side-project, PR #84 merged 2026-08-03)
Standalone tool `openalex_pdf_feed/` — **not** part of the FastAPI app: no Postgres, no shared code with `app/`, own `.github/workflows/openalex-pdf-feed.yml` (independent of `seeder.yml`). Personal automation, not the product: pulls Open Access PDFs from OpenAlex (self-hosts PDF/TEI XML for 60M OA works since 2026-02-24, `content.openalex.org/works/{id}.pdf`) by a curated exact-phrase dictionary (`terms.yaml` — unquoted phrases, code quotes them; unquoted `title_and_abstract.search` gives up to 42x noise via stemming) into a dedicated Cloudflare R2 bucket. Discovery also filters `type:article|review` (excludes preprints for free — separate, mutually-exclusive `type` value), `language:en|ru|fr`, `is_retracted:false` (not `!true` — invalid syntax for boolean fields, verified live against the API). Idempotency: the R2 bucket itself is dedup state (`HEAD papers/<id>.pdf`) — no separate manifest, since the GitHub Actions runner is ephemeral. Delivery to the end user's Mac via `rclone`+`launchd` (README in that dir), not a code path into this repo — `mac_publish.py` additionally republishes files Mac-side under human-readable names (R2 itself stays ID-named, dedup untouched). `terms.yaml`/`openalex_pdf_feed/README.md` gitignored since 08-03 (personal details, not for the public repo) — `terms.yaml` materializes in CI from a `OPENALEX_TERMS_YAML` secret, individual phrases masked in logs. Cron temporarily daily (was weekly) during initial history backfill — commit `624a02b` explains revert condition. Zotero Web API auto-push evaluated, explicitly declined (file-storage/subscription concerns) — no dependency added.
