...
## AI NL→pivot — удалено (добавлено PR #64/#65/#66 2026-07-12, убрано PR #78 2026-08-01)
Текст → LLM → параметры pivot-таблицы в Table Builder. Калибровка модели/rate-limit после запуска, явно отложенная на «первую неделю живого трафика», так и не произошла за 3 недели — удалено чистым коммитом (не revert), история и обоснование в памяти `project-ai-nl-pivot`. Общий урок остаётся актуальным для любой будущей фичи, вызывающей OpenRouter с бэкенда: `OPENROUTER_API_KEY` сидера живёт только как GitHub Actions secret — Railway-сервису нужна отдельная env var с тем же именем (прод-инцидент 2026-07-11/12, 100% отказов до ручного фикса).
