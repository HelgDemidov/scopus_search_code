...
| 4.0 | 2026-04-16 | Закрыты все 17 замечаний adversarial-анализа: Tailwind v3, Tremor v3, OAuth popup flow, keyword-фильтр, UserResponse.created_at, ArticleFilters, StatsStore/ArticleStore split, localStorage XSS-примечание, деплой-чеклист |
...
/auth/callback    → Обработчик Google OAuth postMessage (технический маршрут)
...
- Кнопка **«Continue with Google»** (официальный стиль Google Identity
  Guidelines) → открывает OAuth popup (см. ниже)
...
#### Google OAuth flow — popup + postMessage

Бэкенд `/auth/google/callback` возвращает `JSONResponse({access_token, token_type})`
**напрямую браузеру** — без 302-редиректа на фронтенд. Это означает, что
стандартный full-page redirect flow неприменим: браузер получит JSON как страницу,
а не как ответ фронтенду. Решение: **popup + `window.opener.postMessage`**.

```
[Пользователь] → кнопка «Continue with Google»
    → фронтенд: popup = window.open('/api/auth/google/login', '_blank', 'width=500,height=600')
        → GET /auth/google/login (бэкенд, через proxy /api → Railway)
        → 302 редирект на accounts.google.com
        → [Google OAuth consent screen]
        → 302 редирект на GET /auth/google/callback (бэкенд, Railway URL)
        → бэкенд возвращает JSONResponse({access_token, token_type})
            → popup отображает JSON как страницу
                → НО: фронтенд заранее разместил на /auth/callback
                  страницу-relay, которая перехватывает токен через URL
```

**Практическая реализация (рекомендуемый вариант):**

Поскольку `google_callback` возвращает JSON, а не HTML с JS,
нужно изменить бэкенд ИЛИ использовать промежуточную страницу.

**Вариант A — модификация бэкенда (рекомендуется):**
Изменить `google_callback` так, чтобы он делал редирект на фронтенд с токеном
в URL-параметре: `RedirectResponse(url=f"{FRONTEND_URL}/auth/callback?token={jwt_token}")`.
Тогда `/auth/callback` на фронтенде парсит `?token=` из URL и сохраняет токен.

```typescript
// pages/OAuthCallback.tsx — вариант A (бэкенд делает редирект)
const OAuthCallback = () => {
...
**Вариант B — popup + postMessage (если бэкенд не меняется):**
Открыть `/auth/google/login` как popup. Popup окажется на странице с JSON.
Так как это cross-origin (Railway ≠ Vercel), `window.opener.postMessage`
из popup напрямую недоступен. В этом случае требуется relay-страница:
бэкенд `google_callback` должен вернуть HTML-страницу с JS, вызывающей `postMessage`.

> **Решение при реализации:** использовать Вариант A —
> изменить `google_callback` в `auth.py`, чтобы возвращал
> `RedirectResponse` на фронтенд с токеном в query-параметре.
> Это потребует добавления переменной `FRONTEND_URL` в Railway env.
  ...
  theme: {
    extend: {
      ...
      colors: {
        ...
        // chart-цвета — см. §7.2.5: определяются через safelist, не здесь
      },
    },
  },
  plugins: [],
...
2. **Добавить safelist** (чтобы Tailwind purge не удалил классы, которые
   Tremor использует динамически):
...
# .env.production (Vercel environment variables — добавить в .gitignore)
