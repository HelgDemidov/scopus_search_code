# Скрипт для формирования текстовой схемы-дерева файловой структуры проекта
# Запуск из корня репозитория scopus_search_code (PowerShell):
#   python docs/project_tree/scopus_project_tree.py
#
...
#   Copy-Item docs/project_tree/pre-push-hook .git/hooks/pre-push
...
IGNORE_DIRS = {
    ...
    ".solid-cache", ".grimp_cache", ".tmp_heuristics_tests",
    "build", "dist",
}
...
def print_tree(root: Path, file_obj, prefix: str = "") -> None:
    ...
    # Собираем записи: пропускаем служебные директории и игнорируемые файлы
    entries = [
...
if __name__ == "__main__":
    # Скрипт лежит в docs/project_tree/ — поднимаемся до корня репозитория
    # project_tree -> docs -> <repo_root>
    repo_root = Path(__file__).resolve().parents[2]
    script_dir = Path(__file__).resolve().parent

    output_file = script_dir / "scopus_project_tree.txt"
