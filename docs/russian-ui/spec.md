...
### OQ-1: Перевод "Open Access"

"Open Access" — международный термин. Варианты:
- **A (рекомендуется):** Оставить как есть на обоих языках
- **B:** Перевести как "Открытый доступ"

Обсудить с пользователем. Влияет на badge в `ArticleCard`, KPI-тайл, label в filters.

### OQ-2: Авто-детект языка браузера

Текущий план: `LanguageDetector` с приоритетом `localStorage` → `navigator`.  
Вопрос: включать ли авто-переключение на RU для русскоязычных браузеров при первом посещении (до того как пользователь что-либо выбрал)?

- **A (рекомендуется):** Да — `navigator.language` как fallback после `localStorage`. Пользователи с русским браузером видят сайт на русском сразу.
- **B:** Нет — всегда EN по умолчанию, только явный ручной выбор.

### OQ-3: Перевод Zod validation messages vs. оставить EN

Перевод Zod-сообщений требует рефакторинга схем (§6.3 — фабричный паттерн). Это трудоёмко (~30% усилий по AuthPage) и создаёт дополнительные тест-кейсы.

- **A (рекомендуется):** Переводить — пользователь видит форму на русском, а сообщения об ошибках на английском — плохой UX.
- **B:** Оставить validation messages на EN в первой версии, добавить в следующем PR.

### OQ-4: TypeScript типизация ключей

`react-i18next` поддерживает strict typing: `t('nav.explore')` ловит ошибку компиляции при несуществующем ключе.  
Требует настройки `i18next.d.ts` с импортом типов из `translation.json`.

- **A (рекомендуется):** Включить — поможет поддерживать словари синхронизированными.
- **B:** Пропустить для скорости — `t()` без типов работает, ошибки ловятся только в `i18n.test.ts`.

### OQ-5: Locale-aware date format для русского

Текущий формат в ProfilePage: `"02 Jun 2026"` (en-US).  
В русской локали `Intl.DateTimeFormat('ru', { day: '2-digit', month: 'long', year: 'numeric' })` даст: `"02 июня 2026 г."`.

Принять ли автоматическое переключение формата дат, или зафиксировать универсальный формат (DD MMM YYYY)?
