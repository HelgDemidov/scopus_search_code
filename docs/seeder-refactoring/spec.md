...
**Ветка:** `seeder-refactoring` → merged в `main` 2026-06-26
**Дата создания:** 2026-06-26
**Статус:** Завершён, PR #18 смерджен
...
## Итоги рефакторинга (2026-06-26)

**PR #18** смерджен в `main`, все CI-проверки зелёные.

### Что реализовано

| Коммит | Реализовано | Ключевой результат |
|---|---|---|
| 1 | Убран `_EXCLUSION_WINDOW=200`; пул соединений; накопительный счётчик | LLM даёт ~80–100 фраз вместо ~26 |
| 2 | Миграция `0012`: колонка `last_offset` | Applied на production + staging Supabase |
| 3 | `start` в `ISearchClient`, `ScopusHTTPClient`, `/seeder/seed` | Пагинация работает через API |
| 4 | Двухрежимный прогон: Блок A (50 новых) + Блок B (188 ре-пагинаций) | ~238 Scopus-вызовов/прогон |
| 5 | 11 новых тестов (unit + integration); 113 → 124 тестов; coverage ≥ 79% | CI покрывает весь новый функционал |

### Пропускная способность

| Метрика | До | После |
|---|---|---|
| Фраз/прогон от LLM | ~26 | ~80–100 |
| Статей за прогон (потолок) | ~650 | ~5 950 |
| Прогонов в сутки | 12 | 12 |
| Запросов/неделю (из квоты 20k) | ~2 184 | ~2 856 |

### Состояние БД на момент мерджа

- Статей в коллекции: **85 506**
- Фраз в `seeder_keywords`: ~600–700; все получили `last_offset=25` при миграции
- Первый прогон после деплоя: Block B обработает 188 фраз с `start=25` → ожидаемый прирост ~1 000–4 000 статей за прогон

### Пост-мердж доработки

| Коммит | Изменение |
|---|---|
| `62d1d13` | DOI-фильтр в `ScopusHTTPClient.search()`: статьи без `prism:doi` пропускаются на этапе парсинга. 1 766 статей без DOI удалены из production-БД. |

### Что осталось вне рамок

- Фазы 3 (year-range шардинг) — активируется когда первые фразы достигнут `last_offset = 4975`
